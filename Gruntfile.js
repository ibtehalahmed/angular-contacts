...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    i18nextract: {
      default_options: {
        src: ['client/app/**/*.js', 'client/app/**/*.html', 'client/components/**/*.html', 'client/components/**/*.js'],
        lang: ['en-us', 'ar-eg'],
        dest: 'client/i18n',
        namespace: true,
        jsonSrc: ['client/i18n/*.json']
      }
    },
    ...
    filerev: {
      dist: {
        src: [
          '<%= yeoman.dist %>/<%= yeoman.client %>/!(bower_components)!(rtl){,*/}*.{js,css}'
        ]
      }
    },
    ...
    copy: {
      dist: {
        files: [{
          ...
          cwd: '<%= yeoman.client %>/bower_components/bootstrap-rtl/dist/css',
          expand: true,
          dot: true,
          dest: '<%= yeoman.dist %>/<%= yeoman.client %>/assets2/',
          src: ['bootstrap-rtl.min.css']
        },
          {
            cwd: '<%= yeoman.client %>/bower_components/bootstrap-rtl/dist/css',
            expand: true,
          ...
          {
            cwd: '<%= yeoman.client %>/bower_components/open-sans-fontface',
            expand: true,
            dot: true,
          ...
          {
            cwd: '<%= yeoman.client %>/bower_components/icomoon-bower',
            expand: true,
            dot: true,
            dest: '<%= yeoman.dist %>/<%= yeoman.client %>/app',
            src: ['fonts/*.*']
          ...
          },
          {
            expand: true,
            dot: true,
            cwd: '<%= yeoman.client %>/bower_components/font-awesome', // change this for font-awesome
            src: ['fonts/*.*'],
          ...
          },
          {
            expand: true,
            dot: true,
            cwd: '<%= yeoman.client %>/bower_components/bootstrap', // change this for bootstrap
            src: ['fonts/*.*'],
          ...
          },
          {
            expand: true,
            dot: true,
            cwd: '<%= yeoman.client %>/bower_components/jquery-ui/themes/base', // change this for jquery
            src: ['images/*.*'],
            dest: '<%= yeoman.dist %>/<%= yeoman.client %>/app'
          }, {
            expand: true,
    ...
    injector: {
      ...
      css: {
        options: {
          transform: function (filePath) {
            if(filePath.indexOf('rtl')>0) return;
            var yoClient = grunt.config.get('yeoman.client');
            filePath = filePath.replace('/' + yoClient + '/', '');
            filePath = filePath.replace('/.tmp/', '');
